...
public class PageFactory {
	...
	public static String getPage(int cPage, int numPerpage, int totalData, String url, String function) {
		...
		if (pageNo == 1) {
			...
			pageBar.append("<a class='page-link' href='javascript:" + function + "(" + (pageNo - 5) + ")'>&lt;");
			pageBar.append("</a>");
			pageBar.append("</li>");
		}
		...
		while (!(pageNo > pageEnd || pageNo > totalPage)) {
			if (pageNo == cPage) {
				...
				pageBar.append("<li class=''>");
				pageBar.append("<a class='' href='javascript:" + function + "(" + (pageNo) + ")'>" + pageNo);
				pageBar.append("</a>");
				pageBar.append("</li>");
			}
			pageNo++;
		...
		if (pageNo > totalPage) {
			...
			pageBar.append("<li class=''>");
			pageBar.append("<a class='' href='javascript:" + function + "(" + (pageNo) + ")'>&gt;");
			pageBar.append("</a>");
			pageBar.append("</li>");
		}
